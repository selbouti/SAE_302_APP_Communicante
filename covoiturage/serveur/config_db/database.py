# covoiturage/serveur/config_db/database.py
import sqlite3
...
def migrate_db(conn):
    """Migrer la DB pour ajouter les colonnes manquantes / tables nouvelles"""
    c = conn.cursor()
    ...
    if not c.fetchone():
        # Table inexistante → création complète
        print("✓ Création table voitures (format SAE)...")
        ...
        # Table existante → migration douce
        for col, col_type in [
            ("marque", "TEXT NOT NULL"),
            ("modele", "TEXT NOT NULL"),
        ...
        # Assurer unicité sur utilisateur_id
